...
public class StringTextUtils {
	 ...
	 */
	public static MutableComponent toText(String unformatted) {
	 ...
	 */
	public static String reorder(FormattedCharSequence renderable, boolean includeStyleData) {
	...
	public static String getFormattingCodes(Style style) {
		...
		codes.append(
			style.getColor() != null
				? (color = ChatFormatting.getByName(style.getColor().serialize())) != null
					? "&" + color.getChar()
					: "&" + formatValue(style.getColor())
				: "&r"
		);
	...
	private static String formatValue(TextColor color) {
